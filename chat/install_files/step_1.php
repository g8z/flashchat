...
define('CHATUI_MD5', '545e24c6507871535aa1964ffdc1a84e');
...
/*	$dirEnabled = $canContinue = isWriteable ( $canContinue, './temp/modules',  0777, '/temp/modules' );
...
{
	$entry_d = dir('./temp/modules/'.$entry);
	while($mod_name = $entry_d->read())
	{
		if(strpos($mod_name, '.xml') !== false)
		{
			$canContinue = isWriteable ( $canContinue, './temp/modules/'.$entry.'/'.$mod_name,  0777, '/temp/modules/'.$entry.'/'.$mod_name);
		}
	}
	$entry_d->close();
	$i++;
