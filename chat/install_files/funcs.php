...
function addFontsToConfig($dbpref, $session_instance, $cacheType, $cachePath, $cacheFilePrefix)
{
	...
	{
	    if(!($entry=="." || $entry==".." || strpos($entry, ".swf")===false))
	    {
			$fonts[]=ucfirst(substr($entry, 0, -4));//.swf
		}
	}
	$d->close();
...
function clearTempDir($fname)
{
	...
	while(false!==($entry = $d->read()))
	{
		if('config.srv.php' == $entry || 'modules'==$entry || $entry=="." || $entry==".." || strpos($entry, "_users_")!==false) continue;
...
function fillTempDir($fname)
{
	...
	while(false !== ($entry = $d->read()))
	{
		...
		if(is_file($fname . '/' . $entry))
		{
			...
			// added by Pavel 22.10.09
			if (strpos($new_fname, 'config.srv.php') !== false && file_exists($new_fname)) {
				continue;
			}
			//
